      ...
      <div className="border border-border rounded-lg overflow-hidden">
        ...
        <div className="max-h-[200px] overflow-y-auto">
          {timeSlots.map((slot, idx) => {
            ...
            const isSlotDrop = dropTarget === 'slot';
            const qCfg = linked ? EISENHOWER_CONFIG[getQuadrant(linked)] : null;
            return (
              <div
                key={slot.id}
                className="flex items-center gap-2 px-2 py-1 border-b border-border/50 hover:bg-accent/10 transition-colors"
                onDragOver={e => { e.preventDefault(); e.dataTransfer.dropEffect = 'move'; setDropTarget('slot'); }}
                onDragLeave={onDragLeave}
                onDrop={e => onDropSlot(e, idx)}
              >
                <span className="text-[10px] text-muted-foreground w-24 shrink-0 font-mono">{slot.timeSlot}</span>
                {linked ? (
                  <div className="flex items-center gap-1.5 flex-1">
                    <span className="text-[8px] font-bold px-1 py-0.5 rounded text-white" style={{ background: qCfg!.color }}>
                      {qCfg!.label}
                    </span>
                    <span className={`text-[11px] truncate ${linked.status === 'done' ? 'line-through text-muted-foreground' : ''}`}>
                      {linked.task}
                    </span>
                  </div>
                ) : (
                  <span className="text-[11px] text-muted-foreground/40 flex-1">{slot.title || '—'}</span>
                )}
              </div>
            );
          })}
