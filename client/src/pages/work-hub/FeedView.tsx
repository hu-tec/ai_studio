          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 11 }}>
            <thead>
              <tr style={{ background: '#f8fafc', borderBottom: '2px solid #e2e8f0', position: 'sticky', top: 0, zIndex: 1 }}>
                ...
                <th style={{ padding: '4px 6px', fontSize: 10, fontWeight: 700, color: '#475569', textAlign: 'left' }}>유형</th>
                <th style={{ padding: '4px 6px', fontSize: 10, fontWeight: 700, color: '#475569', textAlign: 'left' }}>부서</th>
                <th style={{ padding: '4px 6px', fontSize: 10, fontWeight: 700, color: '#475569', textAlign: 'left' }}>제목</th>
                <th style={{ padding: '4px 6px', fontSize: 10, fontWeight: 700, color: '#475569', textAlign: 'left' }}>내용</th>
                ...
                return (
                  <tr key={post.post_id}
                    ...
                    <td style={{ padding: '3px 6px' }}>
                      <span style={{ padding: '1px 5px', borderRadius: 6, fontSize: 8, fontWeight: 700, background: pt.bg, color: pt.color }}>{post.data.type}</span>
                    </td>
                    <td style={{ padding: '3px 6px', fontSize: 10, color: '#475569', fontWeight: 500, maxWidth: 80, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                      {buildPathLabel(post.data)}
                    </td>
                    <td style={{ padding: '3px 6px', fontSize: 11, color: '#1e293b', fontWeight: 600, maxWidth: 160, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                      {post.data.pinned && <Pin size={9} color="#F59E0B" style={{ marginRight: 3, verticalAlign: 'middle' }} />}
                      {post.data.title}
                    </td>
                    <td style={{ padding: '3px 6px', fontSize: 10, color: '#64748b', maxWidth: 180, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                      {post.data.content?.split('\n')[0] || '—'}
                    </td>
                    {/* 비고 — 인라인 편집 */}
                    <td style={{ padding: '3px 6px' }} onClick={e => { e.stopPropagation(); if (editingNoteId !== post.post_id) { setEditingNoteId(post.post_id); setEditingNoteVal((post.data as any).note || ''); } }}>
                      {editingNoteId === post.post_id ? (
                        <input value={editingNoteVal} onChange={e => setEditingNoteVal(e.target.value)}
                          onBlur={() => saveNote(post)}
                          onKeyDown={e => { if (e.key === 'Enter') saveNote(post); if (e.key === 'Escape') setEditingNoteId(null); }}
                          autoFocus
                          style={{ width: '100%', padding: '2px 4px', border: '1px solid #3B82F6', borderRadius: 4, fontSize: 10, outline: 'none' }}
                          onClick={e => e.stopPropagation()} />
                      ) : (
                        <span style={{ fontSize: 10, color: (post.data as any).note ? '#475569' : '#cbd5e1', cursor: 'text' }}>{(post.data as any).note || '비고 입력'}</span>
                      )}
                    </td>
                    <td style={{ padding: '3px 6px', fontSize: 9, color: '#64748b' }}>
