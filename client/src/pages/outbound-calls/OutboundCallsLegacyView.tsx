...
export function OutboundCallsLegacyView() {
  ...
  if (viewType === 'full-list') {
    return (
      <div className="flex flex-col h-screen bg-[#f1f3f6] text-slate-900 overflow-hidden font-sans">
        <header className="h-14 bg-white border-b border-gray-200 px-2 flex items-center justify-between shrink-0 shadow-sm z-50">
          ...
          <button 
            ...
            className="flex items-center space-x-2 px-2 py-1 bg-slate-100 hover:bg-slate-200 rounded-md text-xs font-black transition-all"
          >
        ...
        <main className="flex-grow p-2 overflow-hidden flex flex-col">
          <div className="bg-white rounded-md border border-gray-200 shadow-sm overflow-hidden flex flex-col flex-grow">
              ...
              <div className="flex items-center space-x-1">
                ...
                <div className="relative">
                  ...
                  <input 
                    ...
                    className="pl-1 pr-2 py-1 bg-gray-50 border border-gray-100 rounded-md text-xs w-80 focus:ring-2 focus:ring-slate-800 focus:bg-white outline-none transition-all"
                  />
                </div>
              </div>
              <div className="flex items-center space-x-2">
                <button onClick={() => downloadFile('excel')} className="flex items-center space-x-2 px-2 py-1 bg-green-50 text-green-700 rounded-md text-xs font-bold border border-green-100">
                  <FileSpreadsheet className="w-4 h-4" />
                  <span>엑셀</span>
                </button>
                <button onClick={() => downloadFile('word')} className="flex items-center space-x-2 px-2 py-1 bg-blue-50 text-blue-700 rounded-md text-xs font-bold border border-blue-100">
    ...
    <div className="flex flex-col h-screen bg-[#f1f3f6] text-slate-900 overflow-hidden font-sans">
      ...
      {showDetailModal && activeId && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-2 animate-in fade-in duration-200">
          <div className="bg-white w-full max-w-2xl rounded-md shadow-sm overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <div className="flex-grow overflow-y-auto p-1 space-y-1">
              ...
              <div className="border-t border-gray-100 pt-2 space-y-1">
                <div className="space-y-2">
                  ...
                  <div className="bg-slate-50 border border-slate-100 rounded-md p-2 text-xs text-slate-600 font-mono whitespace-pre-wrap leading-relaxed">
                    {formData.history || '기존 기록이 없습니다.'}
                  </div>
                </div>
                <div className="space-y-2">
                  <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest">최근 작성 메모</h4>
                  <div className="bg-amber-50/50 border border-amber-100 rounded-md p-2 text-xs text-amber-900 font-medium leading-relaxed">
            ...
            <div className="p-2 bg-gray-50 border-t border-gray-100 flex justify-end space-x-1">
              <button 
                onClick={() => { setShowDetailModal(false); setMode('edit'); }}
                className="px-2 py-1 bg-slate-800 text-white rounded-md text-xs font-black hover:bg-slate-700 transition-all flex items-center space-x-2"
              >
                <Edit2 className="w-3.5 h-3.5" />
                <span>이 정보 수정하기</span>
              </button>
              <button onClick={() => setShowDetailModal(false)} className="px-2 py-1 bg-white border border-gray-200 text-slate-600 rounded-md text-xs font-bold hover:bg-gray-50 transition-all">
                닫기
              </button>
            </div>
          </div>
      ...
      <header className="h-14 bg-white border-b border-gray-200 px-2 flex items-center justify-between shrink-0 shadow-sm z-50">
        <div className="flex items-center space-x-1">
          <div className="w-9 h-9 bg-slate-800 rounded-md flex items-center justify-center text-white shadow-md shadow-slate-200">
            <Phone className="w-5 h-5" />
          </div>
          <div>
        ...
        <div className="flex items-center space-x-1">
          ...
          <div className="flex bg-slate-100 p-1 rounded-md border border-gray-200">
            <button 
              onClick={() => setViewType('dashboard')}
              className={`flex items-center space-x-2 px-2 py-1.5 rounded-lg text-[10px] font-black transition-all ${viewType === 'dashboard' ? 'bg-white text-slate-800 shadow-sm' : 'text-gray-400 hover:text-gray-600'}`}
            >
            ...
            <button 
              ...
              className={`flex items-center space-x-2 px-2 py-1.5 rounded-lg text-[10px] font-black transition-all ${(viewType as string) === 'full-list' ? 'bg-white text-slate-800 shadow-sm' : 'text-gray-400 hover:text-gray-600'}`}
            >
          ...
          <div className="flex items-center space-x-1.5">
            <button className="flex items-center space-x-1.5 px-2 py-1 bg-white text-slate-600 border border-gray-200 rounded-lg text-xs font-bold hover:bg-gray-50 transition-all">
      ...
      <section className="px-2 pt-2 shrink-0">
        <div className="bg-white rounded-md border border-gray-200 shadow-sm p-1 flex items-center justify-between gap-2 overflow-x-auto no-scrollbar">
          ...
          <div className="flex-grow flex items-center justify-center gap-1 shrink-0">
      ...
      <main className="flex-grow flex p-2 gap-2 overflow-hidden">
        ...
        <section className="w-[320px] flex flex-col gap-2 shrink-0">
          <div className="bg-white rounded-md border border-gray-200 shadow-sm flex flex-col h-1/2 overflow-hidden">
            <div className="px-2 py-1 border-b border-gray-100 flex items-center justify-between bg-gray-50/50">
          ...
          <div className="bg-white rounded-md border border-gray-200 shadow-sm flex flex-col h-1/2 overflow-hidden">
            <div className="px-2 py-1 border-b border-gray-100 flex items-center bg-gray-50/50">
              <h2 className="text-[11px] font-black text-gray-400 uppercase tracking-widest flex items-center">
                <CheckCircle2 className="w-3.5 h-3.5 mr-1.5" /> 처리 결과 선택 ✅
              </h2>
            </div>
            <div className="flex-grow p-2 flex flex-col justify-center space-y-1">
              {STATUS_OPTIONS.map(status => (
                <button
                  key={status}
                  disabled={mode === 'view' || (!activeId && mode !== 'add')}
                  onClick={() => setFormData({...formData, status})}
                  className={`flex items-center justify-between p-1 rounded-md border-2 transition-all ${
        ...
        <section className="flex-grow flex flex-col gap-2 min-w-[500px] overflow-hidden">
          ...
          {/* Top: Work List Table */}
          <div className="flex-grow bg-white rounded-md border border-gray-200 shadow-sm overflow-hidden flex flex-col min-h-[300px]">
            <div className="px-2 py-1 border-b border-gray-100 flex items-center justify-between bg-white sticky top-0 z-20">
              <div className="flex items-center space-x-1">
                <h2 className="text-[11px] font-black text-gray-400 uppercase tracking-widest flex items-center">
                ...
                <div className="relative group">
                  ...
                  <input 
                    ...
                    value={searchTerm}
                    onChange={(e) => setSearchTerm(e.target.value)}
                    className="pl-1 pr-1 py-1.5 bg-gray-50 border border-gray-100 rounded-lg text-[11px] w-48 focus:outline-none focus:ring-2 focus:ring-slate-800 transition-all"
                  />
                </div>
              </div>
              <div className="flex items-center space-x-2">
                ...
                {mode === 'delete' && (
                  <button onClick={handleDelete} className="bg-red-600 text-white px-2 py-1.5 rounded-lg text-xs font-bold hover:bg-red-700 shadow-sm">삭제 실행</button>
                )}
              </div>
            </div>
              ...
              <table className="w-full text-left border-collapse table-fixed">
                ...
                <tbody className="divide-y divide-gray-50">
                  {filteredEntries.map(entry => (
                    ...
                    >
                      <td className="px-2 py-1" onClick={(e) => { e.stopPropagation(); handleSelectOne(entry.id); }}>
                        {mode === 'delete' ? (
          ...
          <div className="bg-white rounded-md border border-gray-200 shadow-sm flex flex-col h-[350px] shrink-0 overflow-hidden relative">
            <div className="px-2 py-1 border-b border-gray-100 flex items-center justify-between bg-slate-800 text-white">
              ...
              {activeId || mode === 'add' ? (
                <div className="h-full flex flex-col">
                  ...
                  <textarea 
                    ...
                    disabled={mode === 'view'}
                    className="flex-grow w-full p-2 bg-gray-50 border border-gray-100 rounded-md text-[13px] text-slate-800 focus:outline-none focus:ring-4 focus:ring-slate-800/5 focus:bg-white disabled:bg-gray-50/50 disabled:text-slate-400 transition-all resize-none leading-relaxed font-medium"
                    placeholder="고객과의 상담 내용을 여기에 상세히 기록하세요. 상담 종료 후 상태값 변경과 함께 저장해주세요."
                  />
                </div>
              ) : (
                <div className="h-full flex flex-col items-center justify-center text-center p-1 bg-gray-50/30 rounded-md border-2 border-dashed border-gray-100">
                  <div className="w-16 h-16 bg-white rounded-full flex items-center justify-center shadow-sm mb-1.5">
        ...
        <section className="w-[380px] flex flex-col gap-2 shrink-0 overflow-hidden">
          <div className="flex-grow bg-white rounded-md border border-gray-200 shadow-sm flex flex-col overflow-hidden">
            <div className="px-2 py-1 border-b border-gray-100 flex items-center justify-between bg-gray-50/50">
              <h2 className="text-[11px] font-black text-gray-400 uppercase tracking-widest flex items-center">
              ...
              {activeId ? (
                <div className="space-y-1">
                  <div className="p-2 bg-slate-50 border border-slate-100 rounded-md">
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">누적 기록</p>
                    <div className="text-[11px] text-slate-600 font-mono whitespace-pre-wrap leading-relaxed">
                      {formData.history || '이전 상담 기록이 없습니다.'}
                    </div>
                  </div>
                  <div className="p-2 border border-dashed border-gray-200 rounded-md bg-gray-50/30">
          ...
          <div className="bg-white rounded-md border border-gray-200 shadow-sm p-2 flex flex-col h-48 shrink-0">
            <h2 className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">상담 빈도 TOP 📊</h2>
...
function ModeButton({ active, onClick, icon, label }: { active: boolean, onClick: () => void, icon: React.ReactNode, label: string }) {
  return (
    <button 
      onClick={onClick}
      className={`flex items-center space-x-2 px-2 py-1 rounded-md text-xs font-black transition-all ${
        active 
...
function SortableHeader({ label, sortKey, config, setConfig, className = "" }: { label: string, sortKey: keyof ContactEntry, config: any, setConfig: any, className?: string }) {
  ...
  return (
    <th onClick={toggle} className={`px-2 py-1 text-[10px] font-black text-gray-400 uppercase tracking-widest cursor-pointer hover:bg-gray-100 transition-colors group ${className}`}>
      <div className="flex items-center space-x-1">
        <span>{label}</span>
      ...
      <div className="space-y-1 max-h-[200px] overflow-y-auto pr-2 no-scrollbar">
        {items.map(item => (
          <button 
            ...
            className={`w-full flex items-center space-x-2 px-2 py-1 rounded-md text-[11px] transition-all ${selected.includes(item) ? 'bg-slate-800 text-white font-bold' : 'bg-gray-50 text-slate-600 hover:bg-gray-100'}`}
          >
...
function EmptyState({ icon, text }: { icon: React.ReactNode, text: string }) {
  return (
    <div className="h-full flex flex-col items-center justify-center text-center p-1 opacity-30 space-y-1">
...
function ModalInfoItem({ label, value, icon }: { label: string, value: any, icon: React.ReactNode }) {
  return (
    <div className="flex items-start space-x-1">
      <div className="w-10 h-10 rounded-md bg-slate-50 border border-slate-100 flex items-center justify-center text-slate-400 shrink-0 shadow-sm">
        {icon}
      </div>
...
export default OutboundCallsLegacyView;
