...
export function Sidebar({ activeCategory, setActiveCategory }: SidebarProps) {
  return (
    <aside className="w-[200px] h-full bg-[#F8F9FA] flex flex-col border-r border-gray-200 shrink-0">
      {/* Sidebar Header */}
      <div className="p-2 border-b border-gray-200 flex items-center gap-1 bg-white">
      ...
      {/* Categories (A-E) */}
      <div className="flex-1 overflow-y-auto no-scrollbar py-1 space-y-1 px-2">
        ...
        {CATEGORY_TYPES.map(cat => (
          <button
            ...
            className={clsx(
              "w-full px-2 py-1 flex items-center gap-1 transition-all rounded-lg group relative",
      ...
      {/* Sidebar Footer */}
      <div className="p-1 border-t border-gray-200 bg-white/50 space-y-1">
...
function SidebarAction({ icon, label }: { icon: React.ReactNode, label: string }) {
  return (
    <button className="w-full flex items-center gap-1 px-2 py-1 text-gray-400 hover:text-gray-900 hover:bg-white rounded-lg transition-all group">
