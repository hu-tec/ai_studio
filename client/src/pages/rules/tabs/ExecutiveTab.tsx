      ...
      <div className="grid grid-cols-4 gap-1.5">
        ...
        <div className="rounded border border-gray-200 bg-white p-1.5">
          <div className="flex items-center gap-1 mb-1">
            ...
            <span className="text-meta font-bold text-violet-700">고정·준고정·선택 비율</span>
          </div>
          ...
          <div className="mt-1 grid grid-cols-3 text-meta text-center">
        ...
        <div className="rounded border border-gray-200 bg-white p-1.5">
          <div className="flex items-center gap-1 mb-1">
            ...
            <span className="text-meta font-bold text-emerald-700">부서별 규정 수 TOP5</span>
          </div>
          <div className="space-y-0.5">
            {deptRanking.slice(0, 5).map((d) => (
              <div key={d.name} className="flex items-center gap-1">
                <span className="w-14 text-meta text-gray-600 truncate">{d.name}</span>
                <div className="flex-1 h-2 bg-gray-100 rounded-full overflow-hidden">
                  <div className="h-full bg-emerald-500" style={{ width: `${(d.total / maxDept) * 100}%` }} />
                </div>
                <span className="w-6 text-right text-meta font-bold text-gray-700">{d.total}</span>
              </div>
            ))}
            {deptRanking.length === 0 && <div className="text-meta text-gray-400">데이터 없음</div>}
          </div>
        </div>
        ...
        <div className="rounded border border-gray-200 bg-white p-1.5">
          <div className="flex items-center gap-1 mb-1">
            ...
            <span className="text-meta font-bold text-violet-700">서비스별 규정 수</span>
          </div>
          <div className="space-y-0.5">
            ...
            {serviceRanking.length === 0 && <div className="text-meta text-gray-400">데이터 없음</div>}
          </div>
        </div>
        ...
        <div className="rounded border border-gray-200 bg-white p-1.5">
          <div className="flex items-center gap-1 mb-1">
            ...
            <span className="text-meta font-bold text-blue-700">전체 규모</span>
          </div>
          <div className="text-meta text-gray-700 space-y-0.5">
          ...
          <NavLink to="/eval-criteria" className="mt-1 block text-center text-meta text-blue-500 hover:underline">
      ...
      <div className="grid grid-cols-4 gap-1.5">
        <div className="col-span-2 rounded border border-gray-200 bg-white overflow-hidden">
          <div className="flex items-center gap-1 px-1.5 py-0.5 bg-gray-50 border-b border-gray-200">
            ...
            <span className="text-meta font-bold text-gray-700">부서 × 규정타입 히트맵</span>
          </div>
        ...
        <div className="col-span-2 rounded border border-gray-200 bg-white overflow-hidden">
          <div className="flex items-center gap-1 px-1.5 py-0.5 bg-gray-50 border-b border-gray-200">
            <Grid3x3 size={11} className="text-gray-600" />
            <span className="text-meta font-bold text-gray-700">직급 × 서비스 규정타입 비교</span>
          </div>
          <div className="overflow-x-auto">
            <table className="min-w-full text-meta">
              <thead>
      ...
      <div className="grid grid-cols-4 gap-1.5">
        <div className="col-span-2 rounded border border-amber-300 bg-amber-50 p-1.5">
          <div className="flex items-center gap-1 mb-1">
            ...
            <span className="text-meta font-bold text-amber-700">공백 자동 감지 ({gaps.length}건)</span>
          </div>
          <div className="text-meta text-amber-600 mb-1">총계 0 또는 고정 규정 3개 미만인 항목</div>
          <ul className="space-y-0.5 max-h-[160px] overflow-y-auto">
            {gaps.length === 0 ? (
              <li className="text-meta text-amber-600">공백 없음 — 모든 그룹이 기본 구성을 갖춤.</li>
            ) : (
              gaps.map((g) => (
                <li key={`${g.bucket}-${g.name}`} className="flex items-center gap-1 text-meta bg-white rounded px-1 py-0.5">
                  <span className="rounded bg-amber-200 px-1 py-px text-meta font-bold text-amber-800">{g.bucket}</span>
                  <span className="font-semibold text-gray-800">{g.name}</span>
                  <span className="ml-auto text-meta text-amber-700">고정 {g.fixed} · 총 {g.total}</span>
                </li>
              ))
            )}
          </ul>
        ...
        <div className="col-span-2 rounded border border-blue-300 bg-blue-50 p-1.5">
          <div className="flex items-center gap-1 mb-1">
            <Compass size={11} className="text-blue-600" />
            <span className="text-meta font-bold text-blue-700">방향 제언 (로드맵)</span>
          </div>
          <ol className="list-decimal pl-4 space-y-0.5 text-meta text-blue-800">
