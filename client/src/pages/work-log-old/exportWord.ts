  ...
  for (const slot of log.timeSlots) {
    ...
    const aiTable = new Table({
      rows: [
        ...
        addRow('1차 프롬프트', (ai as any)?.prompt1 || '', 5),
        addRow('2차 프롬프트', (ai as any)?.prompt2 || '', 5),
        addRow('보안 프롬프트 1', ai?.securityPrompt1 || '', 3),
        addRow('보안 프롬프트 2', ai?.securityPrompt2 || '', 3),
        addRow('문제점', (ai as any)?.problems || '', 3),
        addRow('장점', (ai as any)?.advantages || '', 3),
        addRow('보안 메모', (ai as any)?.securityNote || '', 3),
      ],
      width: { size: 100, type: WidthType.PERCENTAGE },
      layout: TableLayoutType.FIXED,
    });
