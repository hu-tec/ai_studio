...
function SectionButtons({ onChat, onEdit }: { onChat: () => void; onEdit: () => void }) {
  return (
    <div className="flex items-center gap-2">
      <button className="flex items-center gap-1.5 px-3 py-1.5 text-[11px] font-bold text-blue-600 border border-blue-600 rounded-full bg-white hover:bg-blue-50 transition-all active:scale-95">
        <RefreshCw className="w-3.5 h-3.5" /> AI재요청
      </button>
      <button
        onClick={onChat}
        className="flex items-center gap-1.5 px-3 py-1.5 text-[11px] font-bold text-slate-600 border border-slate-300 rounded-full bg-white hover:bg-slate-50 transition-all active:scale-95"
      >
        <MessageSquare className="w-3.5 h-3.5" /> AI챗봇도움
      </button>
      <button
        onClick={onEdit}
        className="flex items-center gap-1.5 px-3 py-1.5 text-[11px] font-bold text-white bg-blue-600 rounded-full hover:bg-blue-700 transition-all active:scale-95 shadow-sm"
      >
...
export function Page2({ onNext, onEdit, onBack }: PageProps) {
  ...
  return (
    <div className="flex flex-col h-full bg-[#F1F5F9]">
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar">
          <div className="max-w-[1000px] mx-auto space-y-6">
              ...
              <div className="p-6 space-y-4">
                <div className="grid grid-cols-6 gap-4 items-center">
                  ...
                  <div className="col-span-5 relative">
                    <input type="text" defaultValue="홍길동(귀하)" className="w-full bg-slate-50 border border-slate-200 rounded-lg px-4 py-2 text-sm focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition-all" />
                ...
                <div className="grid grid-cols-6 gap-4 items-center">
                  ...
                  <div className="col-span-5 relative">
                    <input type="text" defaultValue="900101-1234567" className="w-full bg-slate-50 border border-slate-200 rounded-lg px-4 py-2 text-sm outline-none" />
                    <X className="absolute right-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-slate-300 cursor-pointer" />
                  </div>
                </div>
                <div className="grid grid-cols-6 gap-4 items-center">
                  ...
                  <div className="col-span-5 relative">
                    <input type="text" defaultValue="서울특별시 종로구 세종대로 123" className="w-full bg-slate-50 border border-slate-200 rounded-lg px-4 py-2 text-sm outline-none" />
                    <X className="absolute right-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-slate-300 cursor-pointer" />
                  </div>
                </div>
              </div>
            ...
            <section className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm">
              ...
              <div className="p-6 space-y-4">
                <div className="grid grid-cols-6 gap-4 items-center">
                  <label className="col-span-1 text-[13px] font-bold text-slate-500">성명</label>
                  <div className="col-span-5 relative">
                    <input type="text" defaultValue="임꺽정(가명)" className="w-full bg-slate-50 border border-slate-200 rounded-lg px-4 py-2 text-sm outline-none" />
                    <X className="absolute right-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-slate-300 cursor-pointer" />
                  </div>
                </div>
                <div className="grid grid-cols-6 gap-4 items-center">
                  ...
                  <div className="col-span-5 relative">
                    <input type="text" defaultValue="010-9876-5432" className="w-full bg-slate-50 border border-slate-200 rounded-lg px-4 py-2 text-sm outline-none" />
                    <X className="absolute right-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-slate-300 cursor-pointer" />
                  </div>
                </div>
              </div>
            ...
            <section className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm">
              ...
              <div className="p-6">
                <textarea
                  className="w-full min-h-[120px] bg-slate-50 border border-slate-200 rounded-xl p-4 text-sm text-slate-700 leading-relaxed outline-none focus:border-blue-300 transition-colors"
            ...
            {/* 4. 범죄사실 */}
            <section className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm border-l-4 border-l-orange-500">
              <div className="bg-slate-50 px-6 py-3 border-b border-slate-200 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <h3 className="font-bold text-slate-800 flex items-center gap-2">
              ...
              <div className="p-6">
                <textarea
                  className="w-full min-h-[150px] bg-slate-50 border border-slate-200 rounded-xl p-4 text-sm text-slate-700 leading-relaxed outline-none focus:border-blue-300 transition-colors"
                  defaultValue="피고소인은 2023년 1월경 고소인에게 사업 투자 명목으로 1,000만 원을 편취한 사실이 있습니다. 당시 피고소인은 수익 배분을 약속하였으나..."
                />
                <div className="mt-3 flex items-center gap-2 text-[11px] text-orange-600 bg-orange-50 p-3 rounded-lg border border-orange-100 font-medium">
        ...
        <div className="hidden lg:flex w-[460px] bg-slate-200/50 border-l border-slate-200 flex-col overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto p-8 custom-scrollbar">
            ...
            <div className="bg-white w-full aspect-[1/1.414] shadow-xl rounded-sm p-12 space-y-12 text-slate-800 origin-top">
              <h2 className="text-3xl font-black text-center tracking-[1em] border-b-2 border-slate-900 pb-8">고소장</h2>
              ...
              <div className="pt-20 text-center space-y-4">
                ...
                <p className="text-lg font-black mt-8">종로 경찰서 귀중</p>
              </div>
            </div>

            {/* Next Page Preview */}
            <div className="bg-white w-full aspect-[1/1.414] shadow-lg rounded-sm p-12 mt-8 opacity-50">
      ...
      {/* Bottom Action Bar */}
      <div className="bg-white border-t border-slate-200 p-4 shadow-[0_-4px_12px_rgba(0,0,0,0.03)]">
        <div className="max-w-[1600px] mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            <button
              onClick={onNext}
              className="flex items-center gap-2 px-10 py-3 bg-blue-600 text-white font-black rounded-xl hover:bg-blue-700 transition-all shadow-lg shadow-blue-600/20 active:scale-95"
            >
              다음 단계로
