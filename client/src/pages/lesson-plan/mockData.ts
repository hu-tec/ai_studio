...
export async function fetchLessonPlansFromAPI(): Promise<LessonPlan[] | null> {
  try {
    const res = await fetch('/api/lesson-plans');
    if (!res.ok) return null;
    const rows: { plan_id: string; data: LessonPlan }[] = await res.json();
    if (!Array.isArray(rows) || rows.length === 0) return null;
    return rows.map(r => (typeof r.data === 'object' ? r.data : r) as LessonPlan);
  } catch {
    return null;
  }
}

export async function saveLessonPlanToAPI(plan: LessonPlan): Promise<boolean> {
  try {
    const planId = `plan-${plan.id}`;
    const res = await fetch('/api/lesson-plans', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ plan_id: planId, data: plan }),
    });
    return res.ok;
  } catch {
    return false;
...
export async function deleteLessonPlanFromAPI(planId: number): Promise<boolean> {
  try {
    const res = await fetch(`/api/lesson-plans/plan-${planId}`, { method: 'DELETE' });
    return res.ok;
  } catch {
    return false;
  }
