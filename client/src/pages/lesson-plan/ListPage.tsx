...
import { mockLessonPlans, fetchLessonPlansFromAPI } from "./mockData";
import type { LessonPlan } from "./mockData";
...
export function ListPage() {
  ...
  useEffect(() => {
    let cancelled = false;
    (async () => {
      const apiPlans = await fetchLessonPlansFromAPI();
      if (!cancelled && apiPlans && apiPlans.length > 0) {
        setPlans(apiPlans);
      }
    })();
            ...
            <tbody>
              {paginated.length === 0 ? (
                ...
                paginated.map((lp, idx) => (
                  <tr
                    key={lp.id}
                    onClick={() => navigate(`/lesson-plan/edit/${lp.id}`)}
