...
export const KpiTable: React.FC<KpiTableProps> = ({ data, headers, title, emoji, mode, onUpdate }) => {
  return (
    <div className="w-full overflow-hidden rounded-md border border-slate-200 bg-white">
      <div className="bg-slate-50 px-2 py-1 border-b border-slate-200 flex items-center gap-2">
        ...
        <table className="w-full text-left text-sm border-collapse">
          <thead>
            <tr className="border-b border-slate-100">
              {headers.map((header) => (
                <th key={header} className="px-2 py-1 font-medium text-slate-500 bg-slate-50/50">
                  {header}
                </th>
              ))}
            </tr>
          ...
          <tbody className="divide-y divide-slate-100">
            {data.map((row, rowIndex) => (
              <tr key={rowIndex} className="hover:bg-slate-50 transition-colors">
                {headers.map((header) => (
                  <td key={`${rowIndex}-${header}`} className="px-2 py-1 text-slate-700">
                    {mode === 'edit' ? (
                      <input
                        type="text"
                        value={row[header] || ''}
