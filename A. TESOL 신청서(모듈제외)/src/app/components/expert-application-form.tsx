...
export function ExpertApplicationForm({
  ...
  const [name, setName] = useState("김지예");
  const [email, setEmail] = useState("jiyelab@example.com");
  const [phone, setPhone] = useState("010-1234-5678");
  const [nationality, setNationality] = useState<string[]>(nationalityOptions.map(o => o.label));
  const [discoveryChannels, setDiscoveryChannels] = useState<string[]>(discoveryOptions.map(o => o.label));
  const [course, setCourse] = useState("");
  const [job, setJob] = useState("");
  const [introduction, setIntroduction] = useState(
    "교육 참여 목적과 학습 목표를 간단히 적어주세요."
  );
  const [privacyConsent, setPrivacyConsent] = useState(true);

  const [applicationType, setApplicationType] = useState("수업신청");
  const [region, setRegion] = useState<string[]>(["서울 강남"]);
  const [scheduleDate1, setScheduleDate1] = useState(new Date().toISOString().split("T")[0]);
  const [scheduleDate2, setScheduleDate2] = useState("");
  const [scheduleTime1, setScheduleTime1] = useState("10:00");
  ...
  const resetForm = () => {
    setCurrentStep(1);
    setName("김지예");
    setEmail("jiyelab@example.com");
    setPhone("010-1234-5678");
    setBirthDate("");
    setNationality(nationalityOptions.map(o => o.label));
    setCourse("");
    setJob("");
    setDiscoveryChannels(discoveryOptions.map(o => o.label));
    setIntroduction("교육 참여 목적과 학습 목표를 간단히 적어주세요.");
    setPrivacyConsent(true);
    setApplicationType("수업신청");
    setRegion(["서울 강남"]);
    setScheduleDate1(new Date().toISOString().split("T")[0]);
    setScheduleDate2("");
