...
import { FixedRulesColumn } from "./components/fixed-rules-column";
import { SemiFixedColumn } from "./components/semi-fixed-column";
import { OptionalColumn } from "./components/optional-column";
import { SubRuleSection } from "./components/sub-rule-section";
import {
  categoryTree,
  generateRuleSet,
  type RuleSet,
  type RuleField,
} from "./components/rule-data";
...
export default function App() {
  ...
  // ---- Actions ----
  const handleSave = () => {
    toast.success("규정이 저장되었습니다.", { duration: 2000 });
    setHasChanges(false);
  };
  ...
  const handleReset = () => {
    ...
    setHasChanges(false);
    toast.info("변경사항이 초기화되었습니다.", { duration: 2000 });
  };
