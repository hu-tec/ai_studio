  ...
  return (
    <div className="flex flex-col">
      {items.map((item) => (
        <div
          key={item.id}
          className="flex items-start justify-between border-b border-gray-200 py-2 px-3 gap-2"
        >
...
export function GuidelineGrid({
  ...
  return (
    <div className="grid grid-cols-3 border border-gray-300">
      {/* Column Headers */}
      <div className="bg-gray-800 text-white px-4 py-3 border-r border-gray-300">
        <div>[규정] 고정 (변경금지)</div>
      </div>
      <div className="bg-gray-400 text-black px-4 py-3 border-r border-gray-300">
        <div>[준규정] 준고정 (조건부변경가능)</div>
      </div>
      <div className="bg-gray-200 text-black px-4 py-3 border border-gray-400 border-t-0 border-b-0 border-r-0">
