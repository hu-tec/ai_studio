...
function createInitialState(): RulesState {
  return {
    ...
    ranks: {
      신입: {
        규정: [
          makeItem("입사 후 2주 이내 OJT 교육을 이수해야 한다"),
          makeItem("수습 기간(3개월) 동안 월간 업무 보고서를 제출해야 한다"),
          makeItem("멘토 배정을 받고 주 1회 면담에 참여해야 한다"),
        ],
        준규정: [
          makeItem("사내 시스템 사용법 교육에 참여한다"),
          makeItem("부서 내 업무 프로세스 문서를 숙지한다"),
        ],
        선택사항: [
          makeItem("사내 동호회에 가입할 수 있다"),
          makeItem("자기계발 지원 프로그램을 신청할 수 있다"),
        ],
      },
      사원: {
        규정: [
          makeItem("담당 업무에 대한 주간 보고서를 제출해야 한다"),
          makeItem("연간 필수 교육을 이수해야 한다"),
        ],
        준규정: [
          makeItem("업무 개선 제안을 분기별 1건 이상 제출한다"),
          makeItem("팀 내 지식 공유 활동에 참여한다"),
        ],
        선택사항: [
          makeItem("외부 교육 프로그램에 참여할 수 있다"),
        ],
      },
      주임: {
        규정: [
          makeItem("신입 사원 OJT 멘토링에 참여해야 한다"),
          makeItem("담당 프로젝트의 진행 현황을 주간 보고해야 한다"),
        ],
        준규정: [
          makeItem("팀 내 업무 매뉴얼 갱신에 기여한다"),
        ],
        선택사항: [
          makeItem("사내 강의/세미나 진행을 신청할 수 있다"),
        ],
      },
      대리: {
        규정: [
          makeItem("팀 내 업무 배분 및 조율에 참여해야 한다"),
          makeItem("프로젝트 중간 보고서를 작성해야 한다"),
        ],
        준규정: [
          makeItem("후배 직원 업무 코칭을 실시한다"),
          makeItem("부서 간 협업 회의에 참석한다"),
        ],
        선택사항: [
          makeItem("리더십 교육 프로그램에 참여할 수 있다"),
        ],
      },
      과장: {
        규정: [
          makeItem("팀 내 프로젝트 관리 책임을 수행해야 한다"),
          makeItem("분기별 팀 성과 보고서를 작성해야 한다"),
        ],
        준규정: [
          makeItem("팀원 인사평가에 참여한다"),
          makeItem("예산 집행 현황을 월간 보고한다"),
        ],
        선택사항: [
          makeItem("외부 네트워킹 활동에 참여할 수 있다"),
        ],
      },
      차장: {
        규정: [
          makeItem("부서 업무 기획 및 전략 수립에 참여해야 한다"),
          makeItem("팀장 부재 시 업무를 대행해야 한다"),
        ],
        준규정: [
          makeItem("부서 간 이슈 조율 역할을 수행한다"),
        ],
        선택사항: [
          makeItem("경영 전략 세미나에 참여할 수 있다"),
        ],
      },
      부장: {
        규정: [
          makeItem("부서 연간 업무 계획을 수립하고 보고해야 한다"),
          makeItem("부서원 인사평가를 최종 확정해야 한다"),
          makeItem("부서 예산을 관리하고 집행 결과를 보고해야 한다"),
        ],
        준규정: [
          makeItem("경영진 회의에 참석하여 부서 현황을 보고한다"),
        ],
        선택사항: [
          makeItem("사내 멘토링 프로그램의 멘토로 참여할 수 있다"),
        ],
      },
      팀장: {
        규정: [
          makeItem("팀 운영 계획을 수립하고 이행해야 한다"),
          makeItem("팀원 근태 관리 및 승인을 처리해야 한다"),
          makeItem("팀 성과 목표를 설정하고 관리해야 한다"),
        ],
        준규정: [
          makeItem("팀 내 업무 프로세스 개선을 주도한다"),
          makeItem("팀원 경력 개발 상담을 실시한다"),
        ],
        선택사항: [
          makeItem("타 팀과의 합동 프로젝트를 기획할 수 있다"),
        ],
      },
      이사: {
        규정: [
          makeItem("사업부 전략 수립 및 실행을 총괄해야 한다"),
          makeItem("이사회 보고 자료를 작성해야 한다"),
        ],
        준규정: [
          makeItem("주요 계약 건에 대한 최종 검토를 수행한다"),
        ],
        선택사항: [
          makeItem("외부 자문 위원 활동에 참여할 수 있다"),
        ],
      },
      임원: {
        규정: [
          makeItem("회사 경영 전략 의사결정에 참여해야 한다"),
          makeItem("연간 경영 실적 보고를 수행해야 한다"),
          makeItem("주요 인사 결정에 참여해야 한다"),
        ],
        준규정: [
          makeItem("산업 동향 분석 및 전략 제안을 수행한다"),
          makeItem("대외 협력 및 네트워킹 활동에 참여한다"),
        ],
        선택사항: [
          makeItem("사내 비전/미션 공유 특강을 진행할 수 있다"),
        ],
      },
    },
  };
  ...
  const updateRule = useCallback(
    (section: "company" | "departments" | "ranks", group: string | null, type: RuleType, id: string, text: string) => {
      setState((prev) => {
        ...
        const rs = section === "company" ? next.company : (section === "departments" ? next.departments : next.ranks)[group!];
        if (rs) {
          const item = rs[type].find((r) => r.id === id);
  ...
  const addAttachment = useCallback(
    (section: "company" | "departments" | "ranks", group: string | null, type: RuleType, id: string, fileName: string) => {
      setState((prev) => {
        ...
        if (rs) {
          const item = rs[type].find((r) => r.id === id);
          if (item) item.attachments.push(fileName);
        }
        return next;
      });
    },
